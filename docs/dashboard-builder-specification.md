...
入力上限は、業種/監視対象120文字、顧客/案件ラベル80文字、営業担当者80文字とする。業種が空または上限超過の場合はAPIを呼ばず、入力を無効状態として示し、ボタン操作時はエラー通知へフォーカスを移す。
...
また、パネルの追加、削除、上下移動ができる。上下移動は編集済みパネル配列を入れ替え、生成前プレビュー、下書き、Grafana dashboard JSONの順序へ即時反映する。先頭の上移動と末尾の下移動は無効化する。

パネル編集一覧は、パネル名、目的、単位、可視化方式の部分一致で絞り込める。検索は編集カードの表示だけを変更し、生成前プレビュー、パネル順序、下書き、Grafanaへ送るパネルデータは変更しない。新規提案、パネル追加、下書き破棄では検索条件を解除する。
...
- 顧客条件、Dashboard folder、パネル案作成、Grafana Cloud作成は左側に常時表示する
- 作成履歴、運用診断、実データソース差し替え、AI故障診断、デモ波形、AIログ解析は折りたたみ可能な補助セクションとする
- 右側上部に `条件入力`、`パネル編集`、`Grafana作成` の3工程を表示する
...
- ページ再読み込み後に有効な下書きがあれば復元し、工程を `パネル編集` に戻す
- `下書き破棄` で現在の提案と保存済み下書きを削除できる
- Grafana Cloudへの作成成功後は保存済み下書きを削除する
- アクセスコード、Grafana token、AI API key、Google認証情報は保存しない
- 下書きが不正、空、24パネルを超える、または保存後7日を超えた場合は復元せず、localStorageから削除する
- 復元前に文字列長、数値項目、可視化方式、TestDataシナリオ、パネル数を検証・正規化し、過度に未来の保存日時も無効とする

### 6.5.3 ブラウザAPI通信の停止条件

営業UIからCloud RunまたはローカルNode.jsサーバーへ送るGET/POSTリクエストは、共通通信処理を使用する。

- 既定タイムアウトは30秒とする
- AbortControllerで待機を終了し、操作ボタンは各処理の`finally`で再度有効化する
- タイムアウト時はネットワークとCloud Runの状態確認後に再実行するよう表示する
- 接続失敗はタイムアウトと区別して表示する
- JSONではない応答はHTTPステータス付きの解析エラーとして表示する
- ダッシュボード作成、デモデータ生成、AI実行などのPOSTは自動再送しない
- POSTの再実行は利用者が作成履歴やGrafana側の結果を確認してから行う
