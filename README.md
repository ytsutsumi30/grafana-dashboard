...
このコマンドは板金加工業者のパネル案を実際に生成し、デスクトップ/モバイルのレイアウト、3工程表示、補助機能の折りたたみ、プレビュー描画、パネル検索・並べ替え、実行時Grafana URL、入力上限とエラーフォーカス、Google OIDC/IAP時の認証UIを確認します。スクリーンショットと結果JSONは `outputs\ui-verification\` に保存します。

パネル案と編集値は同じブラウザへ自動保存され、保存後7日以内はページ更新後に復元されます。検証ループでは、編集、保存、再読み込み、復元、期限切れ削除、アクセスコード非保存、ローカル提案APIも確認します。アプリ改善候補の評価方法と停止条件は `docs/app-improvement-loop.md` に記録しています。

ブラウザからのAPI通信は30秒でタイムアウトし、ネットワークとCloud Runを確認してから再実行する案内を表示します。ダッシュボード作成などのPOSTは重複処理を避けるため自動再送しません。検証ループはタイムアウトと接続失敗も疑似発生させて確認します。
