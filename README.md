...
このコマンドは板金加工業者のパネル案を実際に生成し、デスクトップ/モバイルのレイアウト、3工程表示、補助機能の折りたたみ、プレビュー描画、パネル検索・並べ替え、実行時Grafana URL、入力上限とエラーフォーカスも確認します。スクリーンショットと結果JSONは `outputs\ui-verification\` に保存します。
